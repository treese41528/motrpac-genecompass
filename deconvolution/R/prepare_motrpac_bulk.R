...
# old/any ENSRNOG -> current gene_symbol, via FEATURE_TO_GENE (ensembl_gene column)
fg <- ld(file.path(DATA_DIR, "FEATURE_TO_GENE.rda"))
fok <- !is.na(fg$ensembl_gene) & nzchar(fg$ensembl_gene) & grepl("^ENSRNOG", fg$ensembl_gene) &
       !is.na(fg$gene_symbol)  & nzchar(fg$gene_symbol)
fg2 <- data.frame(ens = fg$ensembl_gene[fok], sym = upper(fg$gene_symbol[fok]),
                  stringsAsFactors = FALSE)
fg2 <- fg2[!duplicated(fg2$ens), ]                 # first symbol per ENSRNOG
sym_by_ens <- setNames(fg2$sym, fg2$ens)
rm(fg, fg2); invisible(gc())
...
process_tissue <- function(tis) {
  ...
  data.frame(tissue = tis, in_genes = length(ids), kept = sum(keep),
             ...
             symbol = sum(lm$method == "symbol"), dropped = sum(lm$method == "unmapped"),
...
summ <- do.call(rbind, lapply(tissues, function(t) {
  s <- process_tissue(t)
  cat(sprintf("  %-7s in=%d kept=%d out=%d (direct=%d symbol=%d drop=%d collapse=%d) samples=%d\n",
              s$tissue, s$in_genes, s$kept, s$out_genes, s$direct, s$symbol,
...
mt <- table(factor(lm_canon$method, levels = c("direct", "symbol", "unmapped")))
...
if (file.exists(trf)) {
  ...
  recov    <- !is.na(orph_sym) & (orph_sym %in% vocab_sym)
  cov_before <- mean(union_tr %in% vocab_ens)
  cov_after  <- (sum(union_tr %in% vocab_ens) + sum(recov)) / length(union_tr)
